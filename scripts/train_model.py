...
def train_logistic_regression(X_train, y_train) -> Pipeline:
    ...
    Without scaling, MonthlyCharges/TotalCharges (tens to thousands) sit
    next to 0/1 dummy columns, and the solver frequently fails to fully
    converge (see the ConvergenceWarning). An unconverged fit can produce
    systematically biased probabilities -- e.g. average predicted churn
    probability drifting far from the actual base rate -- so scaling
    isn't just cosmetic here, it fixes real calibration problems.
    """
    model = Pipeline(
        [
